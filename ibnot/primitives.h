...
public:
    ...
    const Point_2& get_position() const
    {
    ...
    Point_2 compute_centroid() const
    {
        FT sum_area = 0.0;
        Vector_2 sum_vector = CGAL::NULL_VECTOR;
        for (unsigned i = 0; i < nb_pixels(); ++i)
        {
            const Pixel& pixel = get_pixel(i);
            FT area = pixel.compute_area();
            Point_2 centroid = pixel.compute_centroid();
